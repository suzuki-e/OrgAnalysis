...
class ChannelsTest < ApplicationSystemTestCase
  ...
  test "creating a Channel" do
    ...
    fill_in "Created", with: @channel.created
    check "Is archived" if @channel.is_archived
    check "Is private" if @channel.is_private
    fill_in "Name", with: @channel.name
    fill_in "Name normalized", with: @channel.name_normalized
    fill_in "Purpose", with: @channel.purpose
